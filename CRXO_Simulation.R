...
RandCRXO <- function(nrep, Ts, clusters, m, TimeEffsInd, Treat_effect, ICC, CAC){
  ...
  rand_results[2] <- mean(output[3,]-Treat_effect)
  
  #Empirical SE
  rand_results[3] <- sqrt(sum((output[1,]-mean(output[1,]))^2)/(nrep-1))
  rand_results[4] <- sqrt(sum((output[3,]-mean(output[3,]))^2)/(nrep-1))
  
  #Average model SE
  rand_results[5] <- sqrt(sum((output[2,])^2)/(nrep))
  rand_results[6] <- sqrt(sum((output[4,])^2)/(nrep))
  
  #Rejection percentage
  rand_results[7] <-  sum(abs(output[1,])/output[2,]>1.96)/nrep    # Hypothesis test
  rand_results[8] <-  sum(abs(output[3,])/output[4,]>1.96)/nrep
  
  names(rand_results) <- c("Model1:Bias", "Model2:Bias", "Model1:ESE", "Model2:ESE", "Model1:ASE", "Model2:ASE", "Model1:Alpha", "Model2:Alpha")
...
RandCRXO(nrep = 100, Ts = 8, clusters = 10, m = 20, TimeEffsInd = 0, Treat_effect = 0, ICC = 0.05, CAC = 0.95)
